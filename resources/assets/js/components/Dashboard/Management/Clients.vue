...
export default {
  ...
  created() {
    Promise.all([
      ...
      const clients =
        respClients.status === 204 ? [] : respClients.data.slice(0);
      personTypesSource =
        respPersonTypes.status === 204 ? [] : respPersonTypes.data.slice(0);
  ...
  methods: {
    createClientAsTableItem(client, index) {
      const personType = personTypesSource.find(
        personType => personType.id == client.person_type_id
      );
      const object = {
        id: client.id,
        name: client.name,
    ...
    addClient(clientRes) {
      ...
      this.clients.push(
        this.createClientAsTableItem(client, this.clients.length)
      );
    },
    edit(client) {
    ...
    delete(clientToDelete) {
      this.$axios
        .delete(`http://localhost:8000/api/clients/${clientToDelete.id}`)
        .then(() => {
          const index = this.clients.findIndex(
            client => clientToDelete.id === client.id
          );
          this.clients = this.clients
            .slice(0, index)
            .concat(this.clients.slice(index + 1));
        });
    }
  }
};

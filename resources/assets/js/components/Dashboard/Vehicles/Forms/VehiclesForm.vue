...
export default {
  ...
  watch: {
    ...
    brandId(value) {
      this.models = this.modelsSource
        .filter(model => model['vehicle_brand_id'] == value)
        .map(model => ({ value: model.id, label: model.description }));
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    validBeforeSave() {
      ...
      if (this.isFormValid()) {
        ...
        this.$swal({
          title: 'Are you sure?',
          html: `The vehicle <b>${this
            .description}</b> will be ${actionToPerform}.`,
          type: 'warning',
    ...
    save() {
      ...
      actionPerformed.then(res => {
        const actionToNotify = this.edit
          ? 'vehicle-updated'
          : 'vehicle-created';
        this.isSavingModel = false;
        res.data.brand = this.brandsSource.find(
          brand => brand.id == this.brandId
        );
        res.data.model = this.modelsSource.find(
    ...
    getUpdatePromise(body) {
      return this.$axios.put(
        `http://localhost:8000/api/vehicles/${this.$props.vehicle.id}`,
        body
      );
    },
